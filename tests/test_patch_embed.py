...
from vit.patch_embed import ConvNextPatchEmbed2d, PatchEmbed2d
...
class TestConvNextPatchEmbed2d:
    ...
    def test_forward(self, backend, normalization, convnext_patch_size):
        ...
        layer = ConvNextPatchEmbed2d(
            ...
            backend=backend,
            convnext_patch_size=convnext_patch_size,
        ).to(device_type)
    ...
    @pytest.mark.parametrize("normalization", ["LayerNorm", "RMSNorm"])
    def test_forward_bigger_patch_size(self, backend, normalization):
        B, C, H, W = 2, 3, 64, 64
        D_model = 64
        device_type = "cuda" if backend == "te" else "cpu"
